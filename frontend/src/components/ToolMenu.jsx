...
function ToolMenu() {
  ...
  const pdfSections = tools.filter(
    (group) => !group.category.toLowerCase().startsWith("image"),
  );
  const imageSections = tools.filter((group) =>
      ...
      <div className="tool-section">
        ...
        <div className="tool-section-content">
          {pdfSections.length > 0 ? (
            pdfSections.map(renderGroupItems)
          ) : (
            <p>No PDF tools available.</p>
          )}
        </div>
      </div>
      ...
      <div className="tool-section">
        ...
        <div className="tool-section-content">
          {imageSections.length > 0 ? (
            imageSections.map(renderGroupItems)
          ) : (
            <p>No Image tools available.</p>
          )}
        </div>
      </div>
    </section>
